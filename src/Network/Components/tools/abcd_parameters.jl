...
function transformation_dq(ABCD₁, ABCD₂)
    ...
    # The formula below was written by Thomas. When this is used, the resulting ABCD matrices have to be devided by four.
    # UPDATE 04/03/24: Divide the T matrix by two
    # T = 0.5 * [1 -1im;-1im -1]
    # CK = (2/3)*[1 -1/2 -1/2;0 sqrt(3)/2 -sqrt(3)/2]
    # CKinv = [1 0;-1/2 sqrt(3)/2; -1/2 -sqrt(3)/2]

    # a_dq_new = T * CK * a₂ * CKinv * conj(T) + conj(T) * CK * a₁ * CKinv * T
    # b_dq_new = T * CK * b₂ * CKinv * conj(T) + conj(T) * CK * b₁ * CKinv * T
    # c_dq_new = T * CK * c₂ * CKinv * conj(T) + conj(T) * CK * c₁ * CKinv * T
    # d_dq_new = T * CK * d₂ * CKinv * conj(T) + conj(T) * CK * d₁ * CKinv * T
