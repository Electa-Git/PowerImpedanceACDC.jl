...
function passivity(G, omega, title :: String = "Passivity assessment")
    ...
    p_abs = plot(f, passivity_index, linewidth = 3, c = :blue, label = "G(s)")  
    plot!(xlabel = "Frequency [Hz]", ylabel = "Re( lambda( . ) )", title = title, framestyle = :box, xaxis = :log10) 
    plot!(xlims = (minimum(f),maximum(f)),ylims = (minimum(passivity_index),maximum(passivity_index)), legend = :topleft)  #xticks = [10^-3, 10^-2, 10^-1, 10^0, 10^1, 10^2, 10^3, 10^4, 10^5, 10^6]
    plot!(size=(1000,1000))
    display(p_abs)
