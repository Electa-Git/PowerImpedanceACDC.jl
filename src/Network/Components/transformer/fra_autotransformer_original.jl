...
@with_kw mutable struct Autotransformer
    ...
    Xₕₗ :: Union{Int, Float64,Complex{Float64}} = 0      # leakage reactance HL [p.u.]
    Xₕₜ :: Union{Int, Float64,Complex{Float64}} = 0      # leakage reactance HT [p.u.]
    Xₜₗ :: Union{Int, Float64,Complex{Float64}} = 0      # leakage reactance TL [p.u.]
...
function autotransformer(;args...)
    ...
    elem = Element(input_pins = at.pins, output_pins = at.pins, element_value = at, transformation = transformation)
    return element
end
