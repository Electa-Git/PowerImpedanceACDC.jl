...
imp_dc, omega_dc = determine_impedance(net, elim_elements = [:dc_line, :tl1],
        input_pins = Any[(:c1, Symbol(2.2))],
        output_pins = Any[(:c1, Symbol(2.1))], omega_range = (0, 4, 1000))
bode(imp_dc, omega = omega_dc)

imp_dc, omega_dc = determine_impedance(net, elim_elements = [:dc_line],
        input_pins = Any[(:c1, Symbol(1.1))],
        output_pins = Any[:gnd1], omega_range = (0, 4, 1000))
bode(imp_dc, omega = omega_dc)

imp_ac, omega_ac = determine_impedance(net, elim_elements = [:tl1],
