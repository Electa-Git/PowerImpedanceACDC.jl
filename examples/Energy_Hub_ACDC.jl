...
@time if multiTerminalAnalysis
    ...
    for i in 1:length(omega)
        ...
        push!(Y_BUS_DC_old, PowerImpedanceACDC.abcd_to_y(ABCD23))
    end

    Y_BUS_DC = PowerImpedanceACDC.make_y_matrix(net,elim_elements = [:MMC2,:MMC3], input_pins = Any[:NodeDC2,:NodeDC3], omega_range = (omega_min, omega_max, omegaₙ))

    ##### ----- Y_BUS_AC2 ----- #####

    Y_BUS_AC2 =  PowerImpedanceACDC.make_y_matrix(net,elim_elements = [:Zg2,:Zg3],input_pins = Any[:NodeD2,:NodeQ2,:NodeD3,:NodeQ3,:NodeDg2,:NodeQg2,:NodeDg3,:NodeQg3], omega_range = (omega_min, omega_max, omegaₙ))
