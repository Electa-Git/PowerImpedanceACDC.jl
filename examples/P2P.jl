# For debugging 
#include("C:\Users\fcifuent\Documents\HVDCStability\hvdcstability.jl\src\HVDCstability.jl")
#using .HVDCstability
...
net = @network begin
    ...
    MMC1 = mmc(Vᵈᶜ = Vdc, vDCbase = 800.0, vACbase_LL_RMS = 380.0, vPCC_scaling = 380/380, Vₘ = Vm, Lᵣ = 60e-3, Rᵣ = 0.535,
        P_max = 1500.0, P_min = -1500.0, P = -Pref, Q = Qref, Q_max = 500.0, Q_min = -500.0,
    ...
    MMC2 = mmc(Vᵈᶜ = Vdc, vDCbase = 800.0, vACbase_LL_RMS = 380.0, vPCC_scaling = 380/380, Vₘ = Vm, Lᵣ = 60e-3, Rᵣ = 0.535,
        P_max = 1000.0, P_min = -1000.0, P = Pref, Q = Qref, Q_max = 1000.0, Q_min = -1000.0,
        occ = PI_control(Kₚ = 0.7691, Kᵢ = 522.7654),
        ccc = PI_control(Kₚ = 0.1048, Kᵢ = 48.1914),
        pll = PI_control(Kₚ = 0.28, Kᵢ = 12.5664),
...
Z_BUS_DC1, omega = determine_impedance(net, elim_elements = [:MMC1], input_pins = Any[:NodeDC1], output_pins = Any[:gndD], omega_range = (omega_min, omega_max, omegaₙ))
...
#=
@time Z_MMC_DC1, omega = determine_impedance(net, elim_elements = [:CableDC12], input_pins = Any[:NodeDC1], output_pins = Any[:gndD], omega_range = (0, 4, 10000))
...
#p = bode(Z_MMC_DC1, omega = omega)
#writedlm("Z_MMC_DC1.csv",  Z_MMC_DC1, ',')
...
#p = bode(Z_MMC_AC2, omega = omega)
#writedlm("Z_MMC_DC2.csv",  Z_MMC_AC2, ',')
...
p = bode(Y_MMC1, omega = omega)
writedlm("Y_MMC1.csv",  Y_MMC1, ',')
writedlm("omega.csv",  omega, ',')

Zg1DQ_test, omega = determine_impedance(net, elim_elements = [:Zg1,:MMC1], input_pins = Any[:NodeDq1, :NodeQg1], 
output_pins = Any[:NodeD1, :NodeQ1], omega_range = (omega_min, omega_max, omegaₙ)) 
Yg1DQ_test = inv.(Zg1DQ_test)
writedlm("Yg1DQ_test.csv",  Yg1DQ_test, ',')
f = omega/(2*pi)


# MMC = net.elements[:MMC2]
# plot_data(MMC, omega_range = (-1, 4, 1000), scale = :log)
...
p = bode(Y_MMC2, omega = omega)
writedlm("Y_MMC2.csv",  Y_MMC2, ',')
