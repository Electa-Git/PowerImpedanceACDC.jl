...
net = @network begin
    ...
    # CableAC23[2.1] ⟷ CableAC2g2[2.1]  ⟷ NodeD2
    # CableAC23[2.2] ⟷ CableAC2g2[2.2]  ⟷ NodeQ2

    # CableAC23[1.1] ⟷ OHLAC3g3[1.1]  ⟷ NodeD3
    # CableAC23[1.2] ⟷ OHLAC3g3[1.2]  ⟷ NodeQ3
    CableAC23[1.1] ⟷ CableAC2g2[2.1] ⟷ Q2[1.1]  ⟷ NodeD2
    CableAC23[1.2] ⟷ CableAC2g2[2.2] ⟷ Q2[1.2]  ⟷ NodeQ2

    CableAC23[2.1] ⟷ OHLAC3g3[1.1] ⟷ Q3[1.1]  ⟷ NodeD3
    CableAC23[2.2] ⟷ OHLAC3g3[1.2] ⟷ Q3[1.2]  ⟷ NodeQ3

    OWF[2.1] ⟷ CableAC2g2[1.1] ⟷ NodeDg2
    OWF[2.2] ⟷ CableAC2g2[1.2] ⟷ NodeQg2
...
if singleTerminalAnalysis
    ...
    # AC-side
    Z_MMC_AC1, omega = determine_impedance(net, elim_elements = [:CableAC2g2], input_pins = Any[:NodeDg2, :NodeQg2], output_pins = Any[:gndD, :gndQ], omega_range = (omega_min, omega_max, omegaₙ))
    Z_BUS_AC1, omega = determine_impedance(net, elim_elements = [:OWF], input_pins = Any[:NodeDg2, :NodeQg2], output_pins = Any[:gndD, :gndQ], omega_range = (omega_min, omega_max, omegaₙ))
...
@time if multiTerminalAnalysis
    ...
    # We get a match regardless of the node order.
    Y_BUS_AC2_full =  HVDCstability.make_y_matrix(net,elim_elements = [:Zg3],input_pins = Any[:NodeDg3,:NodeQg3,:NodeDg2,:NodeQg2,:NodeD2,:NodeQ2,:NodeD3,:NodeQ3], omega_range = (omega_min, omega_max, omegaₙ))
    no_elim = [i for i in 1:4]
    Y_BUS_AC2=[]
    for i in eachindex(Y_BUS_AC2_full)
        push!(Y_BUS_AC2, HVDCstability.kron(Y_BUS_AC2_full[i],no_elim))
    end

    # Old approach of manually obtaining the Ybus matrix
    Y_BUS_AC2_old = []
    ...
    for i in eachindex(omega)
        ...
        ABCDeq = ABCD3g3 * (ABCD23 * ABCD2g2)
        
        push!(Y_BUS_AC2_old, HVDCstability.abcd_to_y(ABCDeq))
    end
    ...
    # maxdiff = 0.0
    # for i in eachindex(Y_BUS_AC2_old)
    #     if abs(sum(Y_BUS_AC2[i] - Y_BUS_AC2_old[i]))>maxdiff
    #         maxdiff = abs(sum(Y_BUS_AC2[i] - Y_BUS_AC2_old[i]))
    #     end
    # end
