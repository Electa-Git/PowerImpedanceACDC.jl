...
@time if multiTerminalAnalysis
    ...
    # External grid 1, representing UK grid (top right)
    ...
    for i in 1:length(omega)
        Y1 = eval_abcd(net.elements[:MMC1].element_value, 1im*omega[i])              # eval_abcd for MMC gives admittance matrix, see converter.jl
        push!(Y_MMC1, [transpose(Y1[1,:]); transpose(-Y1[2,:]); transpose(-Y1[3,:])])  # Adapt to current direction towards MMC 
        # push!(Y_dc1, Y1[1,1])
    ...
    # ABCD_CableDC23 = []
    # for i in 1:length(omega)
    #     ABCD_12 = HVDCstability.transformation_dc(eval_abcd(net.elements[:CableDC12].element_value, 1im*omega[i]))
    #     push!(ABCD_CableDC12, ABCD_12)
    #     ABCD_23 = HVDCstability.transformation_dc(eval_abcd(net.elements[:CableDC23].element_value, 1im*omega[i]))
    #     push!(ABCD_CableDC23, ABCD_23)  
    # end
    ...
    #     push!(Y_BUS_DC_old, [a12/b12 -1/b12 0; -1/b12 a12/b12+a23/b23 -1/b23; 0 -1/b23 a23/b23])
    # end

    Y_BUS_DC = HVDCstability.make_y_matrix(net,elim_elements = [:MMC1,:MMC2,:MMC3], input_pins = Any[:NodeDC1,:NodeDC2,:NodeDC3], omega_range = (omega_min, omega_max, omegaₙ))

    ##### ----- Y_BUS_AC1 ----- #####
    ##### ----- Y_BUS_AC2 ----- #####

    # Alternative approach of individual ABCD parameter calculations
    # ABCD_OHLAC1g1_DQ = []
    # ABCD_CableAC23_DQ = []
    # ABCD_CableAC2g2_DQ = []
    # ABCD_OHLAC3g3_DQ = []

    Y_BUS_AC1 =  HVDCstability.make_y_matrix(net,elim_elements = [:Zg1], input_pins = Any[:NodeD1,:NodeQ1,:NodeDg1,:NodeQg1], omega_range = (omega_min, omega_max, omegaₙ))
    ...
    # Y_BUS_AC2_old = []
    # ω₀ = 100*π
    # @time for i in 1:length(omega)
    #     # push!(ABCD_OHLAC1g1_DQ, HVDCstability.transformation_dq(eval_abcd(net.elements[:OHLAC1g1].element_value, 1im*omega[i] + 1im*ω₀),
    #     # eval_abcd(net.elements[:OHLAC1g1].element_value, 1im*omega[i] - 1im*ω₀)))
        
    #     # push!(ABCD_CableAC23_DQ, HVDCstability.transformation_dq(eval_abcd(net.elements[:CableAC23].element_value, 1im*omega[i] + 1im*ω₀),
    #     # eval_abcd(net.elements[:CableAC23].element_value, 1im*omega[i] - 1im*ω₀)))

    #     # push!(ABCD_CableAC2g2_DQ, HVDCstability.transformation_dq(eval_abcd(net.elements[:CableAC2g2].element_value, 1im*omega[i] + 1im*ω₀),
    #     # eval_abcd(net.elements[:CableAC2g2].element_value, 1im*omega[i] - 1im*ω₀)))

    #     # push!(ABCD_OHLAC3g3_DQ, HVDCstability.transformation_dq(eval_abcd(net.elements[:OHLAC3g3].element_value, 1im*omega[i] + 1im*ω₀),
    #     # eval_abcd(net.elements[:OHLAC3g3].element_value, 1im*omega[i] - 1im*ω₀)))
    ...
    #     ABCD1g1 = HVDCstability.transformation_dq(eval_abcd(net.elements[:OHLAC1g1].element_value, 1im*omega[i] + 1im*ω₀),
    #     eval_abcd(net.elements[:OHLAC1g1].element_value, 1im*omega[i] - 1im*ω₀))
    #     ABCD23 = HVDCstability.transformation_dq(eval_abcd(net.elements[:CableAC23].element_value, 1im*omega[i] + 1im*ω₀),
    #     eval_abcd(net.elements[:CableAC23].element_value, 1im*omega[i] - 1im*ω₀))
    #     ABCD2g2 = HVDCstability.transformation_dq(eval_abcd(net.elements[:CableAC2g2].element_value, 1im*omega[i] + 1im*ω₀),
    #     eval_abcd(net.elements[:CableAC2g2].element_value, 1im*omega[i] - 1im*ω₀))
    #     ABCD3g3 = HVDCstability.transformation_dq(eval_abcd(net.elements[:OHLAC3g3].element_value, 1im*omega[i] + 1im*ω₀),
    #     eval_abcd(net.elements[:OHLAC3g3].element_value, 1im*omega[i] - 1im*ω₀))

    #     # Original implementation by Thomas
    ...
    #     # Alternative approach based on the original code by Aleksandra
    #     push!(Y_BUS_AC1_old, HVDCstability.abcd_to_y(ABCD1g1))
        
    #     n23 = Int(size(ABCD23, 1)/2)
    #     (a23, b23, c23, d23) = (ABCD23[1:n23,1:n23], ABCD23[1:n23,n23+1:end], ABCD23[n23+1:end,1:n23], ABCD23[n23+1:end, n23+1:end])

        
    #     ng2 = Int(size(ABCD2g2, 1)/2)
    #     (a2g2, b2g2, c2g2, d2g2) = (ABCD2g2[1:ng2,1:ng2], ABCD2g2[1:ng2,ng2+1:end], ABCD2g2[ng2+1:end,1:ng2], ABCD2g2[ng2+1:end, ng2+1:end])

        
    #     ng3 = Int(size(ABCD3g3, 1)/2)
    #     (a3g3, b3g3, c3g3, d3g3) = (ABCD3g3[1:ng3,1:ng3], ABCD3g3[1:ng3,ng3+1:end], ABCD3g3[ng3+1:end,1:ng3], ABCD3g3[ng3+1:end, ng3+1:end])
    ...
    #     # With compensation
    #     Ycomp_DQ = 1/((Lcomp*omega[i]*im)^2+(Lcomp*ω₀)^2) * [Lcomp*omega[i]*im -Lcomp*ω₀; Lcomp*ω₀ Lcomp*omega[i]*im]
    #     push!(Y_BUS_AC2_old, [inv(b23)*a23+inv(b2g2)*a2g2+Ycomp_DQ -inv(b23) -inv(b2g2) M0; -inv(b23) inv(b23)*a23+inv(b3g3)*a3g3+Ycomp_DQ M0 -inv(b3g3); -inv(b2g2) M0 inv(b2g2)*a2g2 M0; M0 -inv(b3g3) M0 inv(b3g3)*a3g3])
